...
import com.application.nuntium.databinding.FragmentBusinessBinding
...
class BusinessFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = FragmentBusinessBinding.bind(
            LayoutInflater.from(context).inflate(R.layout.fragment_business, container, false)
        )
        GlobalScope.launch(Dispatchers.IO) {
            Log.i("business", "message")
            newsViewModel.topHeadlinesBusiness(country, language)
        }
        newsViewModel._businessList.observe(viewLifecycleOwner,
            Observer {
                when (it.size) {
                    10 -> {
                        var list = it
                        ...
                        binding.recyclerView.apply {
                            adapter = HomeChoiceAdapter<articles>(
                                requireContext(),
                                it,
                                5,
                                onItemClicked = { model ->
